...
Whenever possible, Red Hat recommends extending the Universal Developer Image (UDI) rather than building a completely custom image. UDI already includes the dependencies required by supported IDEs and development tooling.
...
# Configure the default shell used in terminals
ENV SHELL=/usr/bin/bash

# Required for OpenShift arbitrary UID support
ENV HOME=${USER_HOME_DIR}

# Required when using Buildah inside containers
