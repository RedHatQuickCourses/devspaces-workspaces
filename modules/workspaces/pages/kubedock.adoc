...
* *Podman Workflow:* Developers can use container tooling such as Podman or applications relying on Docker-compatible APIs. Commands like **podman run** work because Podman communicates with Kubedock through the configured **DOCKER_HOST**.
...
* *Reduced Local Setup Complexity:* Developers using Dev Spaces in combination with Kubedock don’t need to install Podman or manage local container runtimes. Everything is managed by OpenShift, reducing the overhead of local environment setup and configuration.
...
* https://docs.redhat.com/en/documentation/red_hat_openshift_dev_spaces/3.27/html-single/user_guide/user_guide?utm_source=chatgpt.com#con_running-containers-with-kubedock_user_guide[Kubedock^]
...
----
+
You need to add two environment variables to the devfile (`KUBEDOCK_ENABLED` and `DOCKER_HOST`). The first enables Kubedock in the workspace, and the second specifies the endpoint of the Kubedock daemon, allowing Podman and Docker-compatible tools to redirect container requests to Kubedock running inside the workspace. You also need to add an `endpoint` section and declare the port at which the Kubedock daemon should listen.
